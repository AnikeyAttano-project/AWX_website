...
- ✅ **Платёжные системы: Platega + ЮKassa** — абстракция PaymentProvider, выбор активной платёжки в админке, вебхуки /webhook/platega и /webhook/yookassa (статус перепроверяется через API)
...
| URL | Кто шлёт | Что делает |
|---|---|---|
| `POST /webhook/platega` | Platega | Проверка подписи (X-MerchantId/X-Secret) → статус через API → fulfill |
| `POST /webhook/yookassa` | ЮKassa | Приём события → статус через API ЮKassa → fulfill |

Обе проверки статуса выполняются через API провайдера, поэтому поддельный вебхук не подтвердит заказ.
